...
function getZombieSpawn() {
    ...
    if (side === 0) return { x: Math.random() * 400, y: -30 }; // Üst
    if (side === 1) return { x: Math.random() * 400, y: 430 }; // Alt
    if (side === 2) return { x: -30, y: Math.random() * 400 }; // Sol
    return { x: 430, y: Math.random() * 400 }; // Sağ
}
    ...
    setInterval(() => {
        for (let n in rooms) {
            ...
            if(r.started) {
                ...
                Object.values(r.players).forEach(p => {
                    if(p.hp <= 0) {
                        if(r.mode === 'zombi') { io.to(n).emit('gameOver', {msg: "Biri öldü! Oda kapandı."}); delete rooms[n]; }
