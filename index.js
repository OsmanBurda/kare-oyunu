...
io.on('connection', (socket) => {
    ...
    socket.on('move', (dir) => {
        ...
        if (p && p.hp > 0 && r.started) { // Sadece oyun başlayınca hareket
            p.lastDir = dir;
    ...
    setInterval(() => {
        for (let n in rooms) {
            let r = rooms[n];
            // OYUN BAŞLAMASA BİLE OYUNCULARI ÇİZDİR (Kutu gözüksün diye)
            if(r.started) {
                // Mermi ve Zombi Mantığı
                ...
                if(r.mode === 'zombi') {
                    ...
                    r.zombies.forEach(z => {
                        ...
                        if(targets.length > 0) {
                            ...
                            if(Math.hypot(z.x-t.x, z.y-t.y) < 20) t.hp -= 0.2; // Hasar biraz artırıldı
                        }
                    });
                }
                ...
                Object.values(r.players).forEach(p => {
                    if(p.hp <= 0) {
                        ...
                        else { p.hp = (r.mode === 'vs' ? 3 : 10); p.x = (p.team==='red'?40:340); p.y = 190; p.hasFlag=false; }
                    }
                });
            }
            // State her durumda gönderilir (Kutu lobideyken de gözükür)
