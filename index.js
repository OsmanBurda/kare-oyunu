...
app.use(express.static(path.join(__dirname))); // Dosya yolu hatasını çözer
...
io.on('connection', (socket) => {
    // Oda listesini herkese gönder
    const sendLobby = () => {
        io.emit('roomList', Object.keys(rooms).map(name => ({
            name, count: Object.keys(rooms[name].players).length, mode: rooms[name].mode
        })));
    };
    sendLobby();
    ...
    socket.on('fire', () => {
        let r = rooms[socket.roomName]; let p = r?.players[socket.id];
        if(p && p.hp > 0 && Date.now() - p.lastFire > 1000) {
            p.lastFire = Date.now();
            if (r.mode === 'bayrak') { // Bayrak modunda 4 yönlü ateş
                [[0,-8],[0,8],[-8,0],[8,0]].forEach(v => r.bullets.push({x: p.x+10, y: p.y+10, dx: v[0], dy: v[1], owner: socket.id}));
            } else { // Diğer modlarda tek yönlü
                const v = {up:[0,-8], down:[0,8], left:[-8,0], right:[8,0]}[p.lastDir];
                r.bullets.push({x: p.x+10, y: p.y+10, dx: v[0], dy: v[1], owner: socket.id});
            }
        }
    });
    ...
    setInterval(() => {
        for (let n in rooms) {
            ...
            // CAN KONTROLÜ (Eksiye düşmez)
            Object.values(r.players).forEach(p => {
                if(p.hp <= 0) {
                    p.hp = (r.mode === 'vs' ? 3 : 10);
                    p.hasFlag = false;
                    r.flags.red.taken = false; r.flags.blue.taken = false;
